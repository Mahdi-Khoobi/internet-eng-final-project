...
router.get("/search", async (req, res) => {
    try {
        ...
        const minBookCountAvailable = req.query.minBookCountAvailable;
        const maxBookCountAvailable = req.query.maxBookCountAvailable;
        const minCommentsCount = req.query.minCommentsCount;

        const filter = {};
        if (category) filter.categories = { $in: [category] };
        if (author) filter.author = author;
        if (minBookCountAvailable || maxBookCountAvailable) {
            filter.bookCountAvailable = {};
            if (minBookCountAvailable) filter.bookCountAvailable.$gte = minBookCountAvailable;
            if (maxBookCountAvailable) filter.bookCountAvailable.$lte = maxBookCountAvailable;
        }
        if (minCommentsCount) {
            filter.$where = `this.comments.length >= ${minCommentsCount}`;
        }
